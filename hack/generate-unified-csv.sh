...
function help_txt() {
	...
	echo "    BLACKBOX_EXPORTER_IMAGE:    (optional) The blackbox exporter image to integrate with"
...
if [ -z "$NOOBAA_CORE_IMAGE" ] || [ -z "$NOOBAA_DB_IMAGE" ] || \
   [ -z "$ROOK_IMAGE" ] || [ -z "$BLACKBOX_EXPORTER_IMAGE" ] || [ -z "$CSV_VERSION" ] || [ -z "$OCS_IMAGE" ] || [ -z "$OCS_METRICS_EXPORTER_IMAGE" ] || \
