...
func (r *ReconcileStorageCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	storageClusterTypeStr := "StorageCluster"
	if instance.Spec.ExternalStorage.Enable {
		storageClusterTypeStr = "External StorageCluster"
		reqLogger.Info(fmt.Sprintf("%s detected", storageClusterTypeStr))
	}
	...
	if instance.Status.Conditions == nil {
		...
		message := fmt.Sprintf("Initializing %s", storageClusterTypeStr)
	...
	r.phase = ""
	// list of default enusure functions
	ensureFs := []ensureFunc{
	...
	for _, f := range ensureFs {
		...
		if r.phase == statusutil.PhaseClusterExpanding && !instance.Spec.ExternalStorage.Enable {
			instance.Status.Phase = statusutil.PhaseClusterExpanding
			phaseErr := r.client.Status().Update(context.TODO(), instance)
			if phaseErr != nil {
			...
			if instance.Status.Phase != statusutil.PhaseReady {
				instance.Status.Phase = statusutil.PhaseProgressing
				if phaseErr := r.client.Status().Update(context.TODO(), instance); phaseErr != nil {
					reqLogger.Error(phaseErr, "Failed to set PhaseProgressing")
				}
			}
		}
...
func (r *ReconcileStorageCluster) ensureCephCluster(sc *ocsv1.StorageCluster, reqLogger logr.Logger) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			reqLogger.Info(fmt.Sprintf("Creating %s", cephClusterTypeStr))
	...
	if !reflect.DeepEqual(cephCluster.Spec, found.Spec) {
		reqLogger.Info(fmt.Sprintf("Updating spec for %s", cephClusterTypeStr))
	...
	type mapNegativeConditionsFunc func(*[]conditionsv1.Condition, *cephv1.CephCluster)
	var negativeMapFunc mapNegativeConditionsFunc = statusutil.MapCephClusterNegativeConditions
	if sc.Spec.ExternalStorage.Enable {
		// negativeMapFunc for External CephCluster
	}
	...
	if found.Status.State == "" {
		...
		negativeMapFunc(&r.conditions, found)
	}
