...
func (r *StorageClusterReconciler) createCephObjectStores(cephObjectStores []*cephv1.CephObjectStore, instance *ocsv1.StorageCluster) error {
	...
	if err != nil {
		// ScaledObject CRD might not be created yet. So just log the error but don't fail the reconcile.
	...
	for _, cephObjectStore := range cephObjectStores {
		...
		case err == nil:
			...
			for _, scaleObject := range scaledObjects.Items {
				if scaleObject.Spec.ScaleTargetRef.Kind == existing.Kind && scaleObject.Spec.ScaleTargetRef.Name == existing.Name {
