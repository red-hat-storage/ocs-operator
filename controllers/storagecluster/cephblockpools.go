...
func (o *ocsCephBlockPools) addPeerSecretsToCephBlockPool(r *StorageClusterReconciler, storageCluster *ocsv1.StorageCluster, poolName string, poolNamespace string) cephv1.MirroringPeerSpec {
...
func (o *ocsCephBlockPools) reconcileCephBlockPool(r *StorageClusterReconciler, storageCluster *ocsv1.StorageCluster) (reconcile.Result, error) {
	...
	if storageCluster.GetDeletionTimestamp() != nil {
		...
		if !errors.IsNotFound(err) {
			...
			err = r.Delete(r.ctx, cephBlockPool)
			if err != nil {
	...
	_, err = ctrl.CreateOrUpdate(r.ctx, r.Client, cephBlockPool, func() error {
		...
		if storageCluster.Spec.Mirroring.Enabled {
			...
			mirroringPeerSpec := o.addPeerSecretsToCephBlockPool(r, storageCluster, cephBlockPool.Name, cephBlockPool.Namespace)
			cephBlockPool.Spec.PoolSpec.Mirroring.Peers = &mirroringPeerSpec
		}
