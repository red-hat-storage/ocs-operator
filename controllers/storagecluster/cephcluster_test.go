...
func TestStorageClassDeviceSetCreationForHigherReplicaCount(t *testing.T) {

	sc1 := &api.StorageCluster{}
	sc1.Spec.Arbiter.Enable = true
	sc1.Spec.StorageDeviceSets = []api.StorageDeviceSet{
		{
			Name:                "mock-dev",
			Count:               1,
			Replica:             6,
	...
	sc1.Status.FailureDomain = "zone"
	sc1.Status.FailureDomainKey = zoneTopologyLabel
	sc1.Status.FailureDomainValues = []string{"zone1", "zone2", "zone3"}

	sc2 := &api.StorageCluster{}
	sc2.Spec.Arbiter.Enable = true
	sc2.Spec.StorageDeviceSets = []api.StorageDeviceSet{
		{
			Name:                "mock-dev",
			Count:               1,
			Replica:             7,
	...
	sc2.Status.NodeTopologies = &api.NodeTopologyMap{
		Labels: map[string]api.TopologyLabelValues{zoneTopologyLabel: []string{"zone1", "zone2", "zone3"}},
	}
	sc2.Status.FailureDomain = "zone"
	sc2.Status.FailureDomainKey = zoneTopologyLabel
	sc2.Status.FailureDomainValues = []string{"zone1", "zone2", "zone3"}
