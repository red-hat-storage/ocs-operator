...
func (r *StorageClusterReconciler) setDefaultMetadataPoolSpec(poolSpec *rookCephv1.PoolSpec, sc *ocsv1.StorageCluster, specifiedPoolSpecPath []string) {
	...
	// Check if DeviceClass is explicitly set in the unstructured obj else set to the default device class
	if val, found := getValueAtPath(r.unstructuredSC, append(specifiedPoolSpecPath, "deviceClass")); found {
		poolSpec.DeviceClass = val.(string)
	} else if poolSpec.DeviceClass == "" {
