...
func (r *StorageAutoscalerReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	err := r.Get(ctx, client.ObjectKey{Namespace: request.Namespace, Name: request.Name}, storageAutoScaler)
	if err != nil {
	...
	if storageAutoScaler.Status.LastExpansion != nil {
		if time.Since(storageAutoScaler.Status.LastExpansion.CompletionTime.Time).Seconds() < r.ScrapeInterval.Seconds() {
	...
	err = r.Get(ctx, client.ObjectKey{Name: storageClusterName, Namespace: request.Namespace}, storageCluster)
	if err != nil {
