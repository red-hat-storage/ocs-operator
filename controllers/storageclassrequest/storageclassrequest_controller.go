...
func (r *StorageClassRequestReconciler) reconcilePhases() (reconcile.Result, error) {
	...
	storageProfileList := &v1.StorageProfileList{}
	if err := r.list(storageProfileList, client.InNamespace(r.OperatorNamespace)); err != nil {
		return reconcile.Result{}, err
	}

	for _, storageProfile := range storageProfileList.Items {
...
func (r *StorageClassRequestReconciler) reconcileCephBlockPool() error {
	...
	_, err := ctrl.CreateOrUpdate(r.ctx, r.Client, r.cephBlockPool, func() error {
		...
		deviceClass := r.storageProfile.Spec.DeviceClass
		...
		r.cephBlockPool.Spec = rookCephv1.NamedBlockPoolSpec{
			PoolSpec: rookCephv1.PoolSpec{
				...
				Parameters: r.storageProfile.Spec.BlockPoolConfiguration.Parameters,
			},
		}
		return nil
	})
