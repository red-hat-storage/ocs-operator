...
func (r *StorageConsumerReconciler) initPhase() error {
	...
	backwardCompatibleMode := r.storageConsumer.GetAnnotations()[defaults.StorageConsumerBackwardCompatibleAnnotation]

	r.isUpdatedProvider = backwardCompatibleMode == defaults.StorageConsumerBackwardCompatibleModeProvider

	switch backwardCompatibleMode {
	case defaults.StorageConsumerBackwardCompatibleModeProvider:
		// old provider mode
	...
	default:
		r.radosnamespaceName = r.storageConsumer.Name
		r.subVolumeGroupName = r.storageConsumer.Name
	}
