...
func (r *StorageConsumerUpgradeReconciler) reconcileStorageRequest(
	...
	rns := &metav1.PartialObjectMetadata{}
	rns.SetGroupVersionKind(rookCephv1.SchemeGroupVersion.WithKind("CephBlockPoolRadosNamespace"))
	rns.Name = rnsName
	rns.Namespace = storageConsumer.Namespace
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(rns), rns); client.IgnoreNotFound(err) != nil {
		return err
	}
	if rns.UID != "" && removeStorageRequestAsOwner(rns) {
		if err := r.Client.Patch(ctx, rns, client.MergeFrom(rns)); client.IgnoreNotFound(err) != nil {
			return err
		}
	}
	...
	subVolumeGroup := &metav1.PartialObjectMetadata{}
	subVolumeGroup.SetGroupVersionKind(rookCephv1.SchemeGroupVersion.WithKind("CephFilesystemSubVolumeGroup"))
	subVolumeGroup.Name = svgName
	subVolumeGroup.Namespace = storageConsumer.Namespace
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(subVolumeGroup), subVolumeGroup); client.IgnoreNotFound(err) != nil {
		return err
	}
	if subVolumeGroup.UID != "" && removeStorageRequestAsOwner(subVolumeGroup) {
		if err := r.Client.Patch(ctx, rns, client.MergeFrom(subVolumeGroup)); client.IgnoreNotFound(err) != nil {
			return err
		}
	}
...
func removeStorageRequestAsOwner(obj client.Object) bool {
