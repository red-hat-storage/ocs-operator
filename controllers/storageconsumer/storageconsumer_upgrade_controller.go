...
func (r *StorageConsumerUpgradeReconciler) reconcileStorageRequest(
	...
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(rbdStorageRequest), rbdStorageRequest); client.IgnoreNotFound(err) != nil {
		...
		if err := r.Client.List(
			ctx, &cephClientsList,
			client.MatchingFields{util.OwnerUIDIndexName: string(rbdStorageRequest.UID)},
			client.InNamespace(storageConsumer.Namespace),
		); err != nil {
			return err
		}
		...
		for i := range cephClientsList.Items {
			...
			if err := r.reconcilePre4_19CephClient(ctx, cephClient, storageConsumer); err != nil {
				return err
			}
		}
	...
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(cephFsStorageRequest), cephFsStorageRequest); client.IgnoreNotFound(err) != nil {
		...
		if err := r.Client.List(
			ctx, &cephClientsList,
			client.MatchingFields{util.OwnerUIDIndexName: string(cephFsStorageRequest.UID)},
			client.InNamespace(storageConsumer.Namespace),
		); err != nil {
			return err
		}
