...
// +kubebuilder:validation:XValidation:rule="!(self.useExistingOsds == true) || !has(self.resources)",message="resources should not be set when useExistingOsds is true"
// +kubebuilder:validation:XValidation:rule="!(self.useExistingOsds == true) || !has(self.count)",message="count should not be set when useExistingOsds is true"
// +kubebuilder:validation:XValidation:rule="!(self.useExistingOsds == true) || !has(self.volumeClaimTemplate)",message="volumeClaimTemplate should not be set when useExistingOsds is true"
type ManageCephNonResilientPools struct {
	...
	// +kubebuilder:validation:XValidation:rule="self == oldSelf",message="skipCreateStorageclass field cannot be modified once set"
	// +optional
	// +nullable
