...
func handleGet(w http.ResponseWriter, r *http.Request, client client.Client) {
	...
	createdViaOBC, err := checkBucketCreatedViaOBC(client, bucketName)
	if err != nil {
...
func checkBucketCreatedViaOBC(client client.Client, bucketName string) (bool, error) {
	klog.Infof("Checking if bucket '%s' was created via OBC", bucketName)
	
	// ToDo: Implement ObjectBucket check
	// Import the objectbucket.io/v1alpha1 API
	// List ObjectBucket resources cluster-wide
	// Check if any have spec.endpoint.bucketName == bucketName
