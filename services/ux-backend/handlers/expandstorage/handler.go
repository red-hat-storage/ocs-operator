...
func handlePost(w http.ResponseWriter, r *http.Request, client client.Client, namespace string) {
	...
	if ExpandStorage.PoolDetails.VolumeType == "block" {
		...
		// create storageClass
		createCephBlockPoolStorageClass(w, r, client, ExpandStorage.StorageClassDetails.Name, ExpandStorage.PoolDetails.PoolName, ExpandStorage.StorageClassDetails.ReclaimPolicy, ExpandStorage.StorageClassDetails.VolumeBindingMode, ExpandStorage.StorageClassDetails.EnableStorageClassEncryption, ExpandStorage.StorageClassDetails.EncryptionKMSID, namespace, string(storageCluster.UID))
		...
		// create storageClass
		createCephFilesystemStorageClass(w, r, client, ExpandStorage.StorageClassDetails.Name, ExpandStorage.PoolDetails.PoolName, ExpandStorage.StorageClassDetails.ReclaimPolicy, ExpandStorage.StorageClassDetails.VolumeBindingMode, ExpandStorage.PoolDetails.FilesystemName, namespace, string(storageCluster.UID))
	} else {
...
func createCephBlockPoolStorageClass(w http.ResponseWriter, r *http.Request, client client.Client, storageClassName, poolName, reclaimPolicy, volumeBindingMode string, enableEncryption bool, encryptionKMSID, namespace, storageClusterUid string) {
	...
	storageClass := &storagev1.StorageClass{
		...
		Parameters: map[string]string{
			"clusterID":                 storageClusterUid,
			"pool":                      poolName,
			"imageFeatures":             "layering,deep-flatten,exclusive-lock,object-map,fast-diff",
			"csi.storage.k8s.io/fstype": "ext4",
			"imageFormat":               "2",
			"encrypted":                 strconv.FormatBool(enableEncryption),
			"csi.storage.k8s.io/provisioner-secret-name":            util.GenerateCephClientSecretName("rbd", "provisioner", storageClusterUid),
			"csi.storage.k8s.io/provisioner-secret-namespace":       namespace,
			"csi.storage.k8s.io/node-stage-secret-name":             util.GenerateCephClientSecretName("rbd", "node", storageClusterUid),
			"csi.storage.k8s.io/node-stage-secret-namespace":        namespace,
			"csi.storage.k8s.io/controller-expand-secret-name":      util.GenerateCephClientSecretName("rbd", "provisioner", storageClusterUid),
...
func createCephFilesystemStorageClass(w http.ResponseWriter, r *http.Request, client client.Client, storageClassName, poolName, reclaimPolicy, volumeBindingMode string, filesystemName, namespace, storageClusterUid string) {
	allowVolumeExpansion := true
	storageClass := &storagev1.StorageClass{
		ObjectMeta: metav1.ObjectMeta{
			Name: storageClassName,
		...
		Parameters: map[string]string{
			...
			"csi.storage.k8s.io/provisioner-secret-name":            util.GenerateCephClientSecretName("cephfs", "provisioner", storageClusterUid),
			"csi.storage.k8s.io/provisioner-secret-namespace":       namespace,
			"csi.storage.k8s.io/node-stage-secret-name":             util.GenerateCephClientSecretName("cephfs", "node", storageClusterUid),
			"csi.storage.k8s.io/node-stage-secret-namespace":        namespace,
			"csi.storage.k8s.io/controller-expand-secret-name":      util.GenerateCephClientSecretName("cephfs", "provisioner", storageClusterUid),
			"csi.storage.k8s.io/controller-expand-secret-namespace": namespace,
		},
	}
