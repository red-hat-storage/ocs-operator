...
func TestNotify(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, srv *OCSProviderServer) {
				expectedName := "remote-obc-" + getObcHash(string(storageConsumer.UID), obcName, obcNamespace)
		...
		{
			name: "notify failed - obc created - OBC already exists",
			setupServer: func(t *testing.T) *OCSProviderServer {
				scheme, schemeErr := newScheme()
				if schemeErr != nil {
					t.Fatalf("newScheme() error = %v", schemeErr)
		...
		{
			name: "notify failed - obc deleted - obc does not exist",
			setupServer: func(t *testing.T) *OCSProviderServer {
				scheme, schemeErr := newScheme()
				if schemeErr != nil {
					t.Fatalf("newScheme() error = %v", schemeErr)
