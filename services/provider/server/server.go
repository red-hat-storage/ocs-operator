...
func (s *OCSProviderServer) GetStorageClaimConfig(ctx context.Context, req *pb.StorageClaimConfigRequest) (*pb.StorageClaimConfigResponse, error) {
	...
	case ocsv1alpha1.StorageConsumerStateReady:
		...
		if req.StorageClaimName == fmt.Sprintf("%s-ceph-rbd", storageCluster.Name) {
			extR, err = s.getStorageClaimConfigBlock(req, kubeResources,
				storageCluster.Name,
			...
			extR, err = s.getStorageClaimConfigCephFs(req, kubeResources,
				storageCluster.Name,
...
func (s *OCSProviderServer) getStorageClaimConfigBlock(
	...
	for _, resource := range kubeResources {
		...
		case v1.SchemeGroupVersion.WithKind("Secret"):
			...
			provSecName := provSecretName
			nodeSecName := nodeSecretName
		...
		case snapapi.SchemeGroupVersion.WithKind("VolumeSnapshotClass"):
			...
			vscName := fmt.Sprintf("%s-%splugin-snapclass", storageClusterName, util.RbdSnapshotter)
			if vsc.Name == vscName {
				extR = append(extR,
					&pb.ExternalResource{
						Name:        vsc.Name,
		...
		case groupsnapapi.SchemeGroupVersion.WithKind("VolumeGroupSnapshotClass"):
			...
			var vgscName string
			vgscName = fmt.Sprintf("%s-ceph-%s-groupsnapclass", storageClusterName, util.RbdGroupSnapshotter)
...
// SHIM for GetStorageClaimConfig to return response to 4.18 clients
func (s *OCSProviderServer) getStorageClaimConfigCephFs(
	req *pb.StorageClaimConfigRequest,
	kubeResources []client.Object,
	storageClusterName string,
	provSecretName string,
	...
	for _, resource := range kubeResources {
		...
		case snapapi.SchemeGroupVersion.WithKind("VolumeSnapshotClass"):
			vsc := resource.(*snapapi.VolumeSnapshotClass)
			vscName := fmt.Sprintf("%s-%splugin-snapclass", storageClusterName, util.CephfsSnapshotter)
			if vsc.Name == vscName {
				extR = append(extR,
					&pb.ExternalResource{
						Name:        vsc.Name,
