...
func (s *OCSProviderServer) GetStorageClaimConfig(ctx context.Context, req *pb.StorageClaimConfigRequest) (*pb.StorageClaimConfigResponse, error) {
	...
	for _, cephRes := range storageRequest.Status.CephResources {
		...
		case "CephFilesystemSubVolumeGroup":
			...
			clientProfileSpec := &csiopv1a1.ClientProfileSpec{
				CephFs: &csiopv1a1.CephFsConfigSpec{
					SubVolumeGroup: cephRes.Name,
				},
			}
			storageClusters := &ocsv1.StorageClusterList{}
			if err := s.client.List(ctx, storageClusters, client.InNamespace(s.namespace), client.Limit(1)); err != nil {
				return nil, status.Errorf(codes.Internal, "failed to get storage cluster: %v", err)
			}
			if len(storageClusters.Items) < 1 {
				return nil, status.Errorf(codes.Internal, "storagecluster does not exist")
			}
			kernelMountOptions := util.GetCephFSKernelMountOptions(&storageClusters.Items[0])
			// TODO: do we want to always send the options?
			if kernelMountOptions == "ms_mode=secure" {
				parts := strings.Split(kernelMountOptions, "=")
				clientProfileSpec.CephFs.KernelMountOptions = map[string]string{parts[0]: parts[1]}
			}
			...
			extR = append(extR,
				...
				&pb.ExternalResource{
					Kind: "ClientProfile",
					Name: "cephfs",
					Data: mustMarshal(clientProfileSpec)},
			)
		}
	}
