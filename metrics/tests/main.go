...
func NewFlagVars() *FlagVars {
	...
	alertTestFlagSet.StringVar(&fVars.mixinPath, "mixin-path", "", "path to the mixins location")
	...
	alertTestFlagSet.Var(fVars.ruleFiles, "rule-files",
		"comma separated prometheus rule files in jsonnet/json/yaml format (default \"internal.jsonnet\" file in mixin's build directory)")
...
func main() {
	...
	if fVars.mixinPath == "" {
		l.ExitOnErrorf(errors.New("empty mixin path"), "Argument, 'mixin-path', not provided")
	}
	if len(fVars.testTemplates.vars) == 0 {
		_ = fVars.testTemplates.Set(filepath.Join(fVars.mixinPath, "tests", "templates"))
	}
	// promAlertTestTemplateFiles, err := listAllTemplateFiles(fVars.alertTemplatesDir)
	// l.ExitOnErrorf(err, "unable to get template files in the directory: %q", fVars.alertTemplatesDir)
	if len(fVars.ruleFiles.vars) == 0 {
		_ = fVars.ruleFiles.Set(filepath.Join(fVars.mixinPath, "build", "internal.jsonnet"))
	}

	l.Infof("MixinPath         : %s", fVars.mixinPath)
