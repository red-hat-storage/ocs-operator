...
type StorageAutoScalerCollector struct {
	StorageAutoScalerMetadata *prometheus.Desc
...
func (s *StorageAutoScalerCollector) Collect(ch chan<- prometheus.Metric) {
	...
	wg.Add(3) // we are waiting for THREE go routines to finish
	go s.collectStorageAutoScalerMetadata(ch, storageAutoScalers, &wg)
...
func (s *StorageAutoScalerCollector) collectStorageAutoScalerMetadata(ch chan<- prometheus.Metric, storageAutoScalers []*v1.StorageAutoScaler, wg *sync.WaitGroup) {
	defer wg.Done()
...
func NewStorageAutoScalerCollector(opts *options.Options) *StorageAutoScalerCollector {
	...
	return &StorageAutoScalerCollector{
		StorageAutoScalerMetadata: prometheus.NewDesc(
			prometheus.BuildFQName(namespace, "storageautoscaler", "metadata"),
			"Contains StorageAutoScaler details",
