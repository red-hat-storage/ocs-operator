...
func TestRGWTLSConfig(t *testing.T) {
	...
	t.Run("TLS 1.2 profile with rook.io selector sets ciphers and groups", func(t *testing.T) {
		...
		assert.Nil(t, stores[0].Spec.Security.CipherSuites, "TLS 1.2 should not set CipherSuites field")
	...
	t.Run("TLS 1.3 profile with rook.io selector disables TLS 1.2", func(t *testing.T) {
		...
		assert.Contains(t, stores[0].Spec.Security.CipherSuites, "TLS_AES_128_GCM_SHA256")
		assert.Contains(t, stores[0].Spec.Security.CipherSuites, "TLS_AES_256_GCM_SHA384")
		assert.Nil(t, stores[0].Spec.Security.Ciphers, "TLS 1.3 should not set Ciphers field")
