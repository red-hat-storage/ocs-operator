...
func (r *StorageClusterReconciler) createCephRGWRoutes(routes []*routev1.Route, instance *ocsv1.StorageCluster) error {
	for _, route := range routes {
		...
		case err == nil:
			...
			// I need to be able to update Routes. Can anyone tell me why this code exists?
			// Original PR mentions nothing about this: https://github.com/red-hat-storage/ocs-operator/pull/1063
...
func (r *StorageClusterReconciler) newCephRGWRoutes(initData *ocsv1.StorageCluster) ([]*routev1.Route, error) {
	...
	wildcardHost := initData.Spec.ManagedResources.CephObjectStores.Hosting.RouteWildcardHost
	for _, obj := range ret {
		if wildcardHost != "" {
			obj.Spec.Host = wildcardHost
...
func (r *StorageClusterReconciler) getRouteEndpoints(initData *ocsv1.StorageCluster) ([]string, bool, error) {
	...
	for _, route := range routes {
		...
		if len(liveRoute.Status.Ingress) == 0 {
		...
		for _, ing := range liveRoute.Status.Ingress {
