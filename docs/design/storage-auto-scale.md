...
1) Create a controller that [requeueAfter](https://pkg.go.dev/sigs.k8s.io/controller-runtime/pkg/reconcile#Result) every 10 mins.
    ...
    b) If the expansion is not in progress set the status `phase` to `NotStarted` 
...
7) Verify and Alert:
   ...
   2) If the scaling is successful will raise a `Succeeded` alert and update the status of the `StorageAutoScaling` CR with `lastExpansionCompletionTime` and `phase` and also osd count and size.
   3) If it is fails will do a requeue every 1 min and, will raise a `failure` alert if scaling not `Succeeded` with in timeoutSeconds(default:1800) interval.   
...
1) If scaling is just started:
    ...
    3) If the scaling is successful set the `phase` to `completed`.
    4) If the scaling is un-successful requeue every 1 min, we have the 2nd case. 
...
2) If the scaling has already started and its requeue
    ...
    5) If the scaling is in-progress with more than timeoutSeconds(default:1800) interval we will raise a user `failure` alert.
    6) If there as a failure alert, provide a mitigation guide for the user. 
