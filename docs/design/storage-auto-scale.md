...
5) If no-expansion is in progress,
   ...
   1) Check the (status.phase == "InProgress"), if yes move the status phase to succeeded.
...
13) Verify and Alert:
    ...
    1) Verify the Storagecluster whether the new osds are added and scaled in size, for all the device sets.
    
       1) For vertical scaling, Use the above queried osd size from Prometheus and match it with storagecluster.spec..size.
    
       2) For horizontal scaling, Use the above queried osd count from Prometheus and match it with storagecluster.spec..count.
